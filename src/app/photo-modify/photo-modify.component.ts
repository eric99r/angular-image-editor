...
@Component({
  selector: 'photo-modify',
  templateUrl: './photo-modify.component.html',
  styleUrls: ['./photo-modify.component.css']
})

export class PhotoModifyComponent implements OnInit {
  imageUrl: any;
  cropperRes: string;
  showCropper: boolean;
  ...
  async onDownloadClicked(event){
    ...
    let blob = await fetch(this.cropperRes).then(r => r.blob());
